...
class Contact extends Person {
    ...
    /**
     * Выравнивает кривое геокодирование.
     * @param $con
     * @return stdClass - объект с полями - координатами геолокации
     */
    public static function getLoc($con){
        $loc = new stdClass();
        if($con->focus->jjwg_maps_lat_c == 0 and $con->focus->jjwg_maps_lng_c){
            $xxx = file_get_contents('https://maps.googleapis.com/maps/api/geocode/json?key=AIzaSyBxjVvCqSS7ttl43sUOQzwd2reUg7uKERc&address='
                                     . str_replace(" ", '%20', $con->focus->primary_address_street) . '%20'
                                     . str_replace(" ", '%20', $con->focus->primary_address_city) . '%20'
                                     . str_replace(" ", '%20', $con->focus->primary_address_state) . '%20'
                                     . str_replace(" ", '%20', $con->focus->primary_address_postalcode) . '%20'
                                     . str_replace(" ", '%20', $con->focus->primary_address_country));
            $loc = json_decode($xxx)->results[0]->geometry->location;
        }
        else{
            $loc->lat = $con->focus->jjwg_maps_lat_c;
            $loc->lng = $con->focus->jjwg_maps_lng_c;
        }
        return $loc;
    }
	...
	*/
	...
	function get_list_view_data($filter_fields = array()) {
        ...
        /*
         * Объединение полей в listview и subpanel(acc)
		 * -можно сделать прямо в *defs шаблонами.
         */
        if(empty($temp_array['TITLE'])){
            $query = "SELECT title FROM contacts WHERE id = '".$this->id."'";
            $result = $this->db->query($query);
            $tmpbd = $this->db->fetchByAssoc($result);
            $this->title = $tmpbd['title'];
            $temp_array['TITLE'] = $this->title;
        }
        $temp_array['NAME'] = $temp_array['TITLE']." ".$temp_array['NAME']; //а ведь можно прям в вардефе кастомным шаблоном

        /*
         * Большой черный костыль.
         * Так и не нашел, почему дата рождения не достается из базы сама.
         * upd. нашел, можно удалить
         * А нет, опять пригодилось.
         */
        if(empty($temp_array['BIRTHDATE'])) {
            $query = "SELECT birthdate FROM contacts WHERE id = '" . $this->id . "'";
            $result = $this->db->query($query);
            $tmpbd = $this->db->fetchByAssoc($result);
            $this->birthdate = $tmpbd['birthdate'];
            $temp_array['BIRTHDATE'] = $this->birthdate;

            $temp_array['BDATE'] = $this->hasBdate($this->birthdate, true);
        }
        else{
            $temp_array['BDATE'] = $this->hasBdate($this->birthdate);
        }


        if($temp_array['BDATE'] == 1){
            $temp_array['BIRTHDATE'] .= "(Сегодня)";

            /*
            * Где покрасить фон всей строки я нашел, но никакие
            * изменения там не отражались на внешнем виде страницы.
             * Поэтому красим только текст.
             */
			 
            $temp_array['NAME'] = "<font color='navy'>".$temp_array['NAME']."</font>";
            $temp_array['ACCOUNT_NAME'] = "<font color='navy'>".$temp_array['ACCOUNT_NAME']."</font>";
            $temp_array['EMAIL1'] = "<font color='navy'>".$temp_array['EMAIL1']."</font>";
            $temp_array['PHONE_WORK'] = "<font color='navy'>".$temp_array['PHONE_WORK']."</font>";
            $temp_array['ASSIGNED_USER_NAME'] = "<font color='navy'>".$temp_array['ASSIGNED_USER_NAME']."</font>";
            $temp_array['BIRTHDATE'] = "<font color='navy'>".$temp_array['BIRTHDATE']."</font>";
            $temp_array['DATE_ENTERED'] = "<font color='navy'>".$temp_array['DATE_ENTERED']."</font>";
            $temp_array['PRIMARY_ADDRESS_CITY'] = "<font color='navy'>".$temp_array['PRIMARY_ADDRESS_CITY']."</font>";
            $temp_array['PRIMARY_ADDRESS_STATE'] = "<font color='navy'>".$temp_array['PRIMARY_ADDRESS_STATE']."</font>";
        }
	 ...
	 *
	 */
    private function hasBdate($bdate, $from_db = false){
        if($from_db){
            $date = date('Y-m-d');
