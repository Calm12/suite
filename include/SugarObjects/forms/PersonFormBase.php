...
function buildTableForm($rows)
{
	...
	if ($action == 'ShowDuplicates')
	{
		...
		else if (!empty($_POST['return_module']) && !empty($_POST['return_action'])    &&(($_POST['return_action'] == "DetailView" && !empty($_REQUEST['return_id']) ) || $_POST['return_action'] != "DetailView")) {
   ...
   $form .= getPostToForm();
    ...
    foreach($rows as $row){
        ...
        var_dump($form);
        $fp = fopen($_SERVER['DOCUMENT_ROOT']."/TEMP_CRON/asdasd.txt", "w");
        fwrite($fp, 'lil');
        fclose($fp);
		...
		if($rowColor == 'evenListRowS7'){
			$rowColor = 'oddListRowS9';
		}else{
			 $rowColor = 'evenListRowS7';
		}
		$form .= "</tr>";
	}
